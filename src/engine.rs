...
impl Engine {
    ...
    pub fn serialize(&self) -> Vec<u8> {
        ...
        let mut serialized = Vec::with_capacity(data.len() + HASH_PREFIX_LENGTH);
    ...
    pub fn deserialize(&mut self, serialized: &[u8]) -> Result<(), DeserializationError> {
        ...
        let serialized_flat_buffer = &serialized[HASH_PREFIX_LENGTH..];

        // Check the hash to ensure the data isn't corrupted.
        let expected_hash = &serialized[0..HASH_PREFIX_LENGTH];
        let actual_hash = seahash::hash(serialized_flat_buffer).to_le_bytes();
        assert!(expected_hash.len() == HASH_PREFIX_LENGTH);
        if expected_hash != actual_hash {
            println!(
                "Expected hash: {:?}, actual hash: {:?}",
                expected_hash, actual_hash
            );
            return Err(DeserializationError::ValidationError);
        }

        let memory = VerifiedFlatbufferMemory::from_raw(serialized_flat_buffer)
