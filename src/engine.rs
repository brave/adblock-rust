...
use crate::data_format::{DatFilePayload, deserialize_dat_file, serialize_dat_file};
...
impl Engine {
    ...
    ///
    /// The checksum rejects truncated or corrupt files. Files from this crate version skip FlatBuffer
    /// verification so reload stays fast; other versions are still verified. A local rewrite of the
    /// stored file is outside this crate's threat model.
    ///
    ...
    pub fn deserialize(&mut self, serialized: &[u8]) -> Result<(), DeserializationError> {
        ...
        let payload = deserialize_dat_file(serialized)?;
        let memory = load_flatbuffer_memory(payload)?;
...
fn load_flatbuffer_memory(
    payload: DatFilePayload<'_>,
) -> Result<VerifiedFlatbufferMemory, DeserializationError> {
    if payload.crate_version_matches {
        // Same crate version and a matching SeaHash. Skip `fb::root_as_engine`.
        // The checksum catches corruption. It does not stop someone who can rewrite the file.
        Ok(VerifiedFlatbufferMemory::from_raw_unchecked(payload.data))
    } else {
        VerifiedFlatbufferMemory::from_raw(payload.data)
