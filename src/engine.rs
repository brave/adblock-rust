...
use crate::data_format::{deserialize_dat_file, serialize_dat_file, token_from_hash};
...
impl Engine {
    ...
    pub fn serialize(&self) -> Vec<u8> {
        self.serialize_with_token().0
    }

    /// Serializes the `Engine` into a binary format so that it can be quickly reloaded later.
    ///
    /// Also returns a token that can be passed to [`Engine::deserialize_with_token`] to skip
    /// FlatBuffer verification when loading these same bytes.
    pub fn serialize_with_token(&self) -> (Vec<u8>, String) {
    ...
    ///
    /// Always verifies the FlatBuffer payload. Use [`Engine::deserialize_with_token`] when the
    /// same serialized bytes may be loaded more than once, so a later load can skip that
    /// verification.
    ///
    ...
    pub fn deserialize(&mut self, serialized: &[u8]) -> Result<(), DeserializationError> {
        self.deserialize_with_token(serialized, None).map(|_| ())
    }

    /// Deserialize like [`Engine::deserialize`], optionally skipping FlatBuffer verification.
    ///
    /// Returns a token of the form `{crate_version}-{hash}`. The same token is produced by
    /// [`Engine::serialize_with_token`]. Persist it and pass it back as `cached_token` the next
    /// time the same bytes are loaded. If it matches, FlatBuffer verification is skipped; the DAT
    /// header checksum is still checked. A missing or mismatched token falls back to full
    /// verification.
    pub fn deserialize_with_token(
        &mut self,
        serialized: &[u8],
        ...
        let (data, hash) = deserialize_dat_file(serialized)?;
        let token = token_from_hash(hash);
        let token_matches = cached_token.is_some_and(|cached| cached == token);

        // Skip FlatBuffer verification only when the caller presents a token from a previous
        // successful load of this same payload and crate version.
        let memory = if token_matches {
            VerifiedFlatbufferMemory::from_raw_unchecked(data)
        } else {
            VerifiedFlatbufferMemory::from_raw(data)
