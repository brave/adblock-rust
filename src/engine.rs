...
impl Engine {
    ...
    /// Deserialize the `Engine` from the binary format generated by `Engine::serialize`, keeping
    ...
    pub fn deserialize(&mut self, serialized: &[u8]) -> Result<(), DeserializationError> {
        let mut new = Self::from_serialized(serialized)?;

        std::mem::swap(&mut self.resources, &mut new.resources);
    ...
    /// Deserialize a new `Engine` from the binary format generated by `Engine::serialize`.
    ///
    /// Note that the binary format has a built-in version number that may be incremented. There is
    /// no guarantee that later versions of the format will be deserializable across minor versions
    /// of adblock-rust; the format is provided only as a caching optimization.
    pub fn from_serialized(serialized: &[u8]) -> Result<Self, DeserializationError> {
        ...
        let blocker = Blocker::from_context(FilterDataContextRef::clone(&context));
        let cosmetic_cache =
            CosmeticFilterCache::from_context(FilterDataContextRef::clone(&context));

        Ok(Self {
            blocker,
            cosmetic_cache,
            resources: ResourceStorage::default(),
            filter_data_context: context,
        })
    }
}
