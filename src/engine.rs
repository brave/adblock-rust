...
impl Engine {
    ...
    pub fn to_content_blocking(&self) -> Vec<crate::content_blocking::CbRule> {
        ...
        let domains_vec: Vec<String> = unique_domains_strings.iter().map(|s| s.to_string()).collect();
        ...
        for list_id in 0..NetworkFilterListId::Size as usize {
            ...
            for filter_fb in list_fb.filter_map_values().iter() {
                ...
                if let Ok(equivalent) = crate::content_blocking::fb_network_filter_to_cb_rule(
                    &filter_fb,
                    &domains_vec,
                ) {
