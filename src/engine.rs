...
impl Default for Engine {
    fn default() -> Self {
        ...
        Self {
            ...
            cosmetic_cache: CosmeticFilterCache::new(filter_data_context.clone()),
...
impl Engine {
    ...
    pub fn from_filter_set(set: FilterSet, optimize: bool) -> Self {
        ...
        for filter in cosmetic_filters {
            ...
            if !filter.has_hostname_constraint() {
                if let Some(selector) = filter.plain_css_selector() {
                    if selector.starts_with('.') {
                        if let Some(key) = crate::cosmetic_filter_cache::key_from_selector(selector)
                        {
            ...
            other_cosmetic_filters.push(filter);
        }

        let memory =
            FlatBufferBuilder::make_flatbuffer(network_filters, simple_class_rules, optimize);

        let filter_data_context = FilterDataContext::new(memory);

        Self {
            blocker: Blocker::from_context(filter_data_context.clone()),
            cosmetic_cache: CosmeticFilterCache::from_rules_with_context(
                other_cosmetic_filters,
    ...
    pub fn deserialize(
        ...
        let (context, cosmetic_cache) = crate::data_format::deserialize_engine(serialized)?;
