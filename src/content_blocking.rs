...
pub fn fb_network_filter_to_cb_rule(
    ...
    let load_type = if mask.contains(NetworkFilterMask::THIRD_PARTY | NetworkFilterMask::FIRST_PARTY) {
        vec![]
    ...
    let url_filter = match (fb_filter.patterns(), fb_filter.hostname()) {
        ...
        (Some(patterns), None) if !patterns.is_empty() => {
            ...
            let mut url_filter = if mask.contains(NetworkFilterMask::IS_LEFT_ANCHOR) {
                ...
                let scheme_part = if mask.contains(NetworkFilterMask::FROM_HTTP | NetworkFilterMask::FROM_HTTPS) {
                    ""
        ...
        _ => if mask.contains(NetworkFilterMask::FROM_HTTP | NetworkFilterMask::FROM_HTTPS) {
            "^https?://".to_string()
        } else if mask.contains(NetworkFilterMask::FROM_HTTP) {
            "^http://".to_string()
        } else if mask.contains(NetworkFilterMask::FROM_HTTPS) {
    ...
    let (if_domain, unless_domain) = if fb_filter.opt_domains().is_some() || fb_filter.opt_not_domains().is_some() {
            ...
            for domain_index in opt_domains {
                if let Some(domain_str) = unique_domains_strings.get(domain_index as usize) {
                    let normalized_domain = if domain_str.is_ascii() {
                        ...
                        idna::domain_to_ascii(&domain_str.to_lowercase()).unwrap_or_else(|_| domain_str.to_lowercase())
                    };
...
impl TryFrom<NetworkFilter> for CbRuleEquivalent {
    ...
    fn try_from(v: NetworkFilter) -> Result<Self, Self::Error> {
        ...
        if let Some(raw_line) = &v.raw_line {
            ...
            let (if_domain, unless_domain) = if v.opt_domains_strings.is_some() || v.opt_not_domains_strings.is_some() {
                let mut if_domain = vec![];
                let mut unless_domain = vec![];
