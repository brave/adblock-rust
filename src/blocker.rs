...
pub struct Blocker<FilterListType: NetworkFilterListTrait> {
    pub(crate) csp: FilterListType,
    pub(crate) exceptions: FilterListType,
    pub(crate) importants: FilterListType,
    pub(crate) redirects: FilterListType,
    pub(crate) removeparam: FilterListType,
    pub(crate) filters_tagged: FilterListType,
    pub(crate) filters: FilterListType,
    pub(crate) generic_hide: FilterListType,
...
impl<FilterListType: NetworkFilterListTrait> Blocker<FilterListType> {
    ...
    fn apply_removeparam(
        removeparam_filters: &FilterListType,
        request: &Request,
    ...
    pub fn new(network_filters: Vec<NetworkFilter>, options: &BlockerOptions) -> Blocker {
        ...
        Blocker {
            csp: FilterListType::new(csp, options.enable_optimizations),
            exceptions: FilterListType::new(exceptions, options.enable_optimizations),
            importants: FilterListType::new(importants, options.enable_optimizations),
            redirects: FilterListType::new(redirects, options.enable_optimizations),
            // Don't optimize removeparam, since it can fuse filters without respecting distinct
            // queryparam values
            removeparam: FilterListType::new(removeparam, false),
            filters_tagged: FilterListType::new(Vec::new(), options.enable_optimizations),
            filters: FilterListType::new(filters, options.enable_optimizations),
            generic_hide: FilterListType::new(generic_hide, options.enable_optimizations),
    ...
    fn tags_with_set(&mut self, tags_enabled: HashSet<String>) {
        ...
        let filters: Vec<NetworkFilter> = self
            ...
            .cloned()
            .collect();
        self.filters_tagged = NetworkFilterListTrait::new(filters, self.enable_optimizations);
    }
