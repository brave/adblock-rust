...
use crate::flatbuffers::containers::flat_multimap::{
    FlatMapStringView, FlatMapView, FlatMultiMapBuilder,
};

use crate::flatbuffers::containers::flat_serialize::{Builder, FlatSerialize};
...
impl<'a, B: Builder<'a>> FlatSerialize<'a, B> for HostnameRule {
    type Output = WIPOffset<fb::HostnameSpecificRules<'a>>;

    fn serialize(&self, builder: &mut B) -> flatbuffers::WIPOffset<fb::HostnameSpecificRules<'a>> {
        let unhide = FlatSerialize::serialize(&self.unhide, builder);
        let uninject_script_vec = FlatSerialize::serialize(&self.uninject_script, builder);
        let procedural_action_vec = FlatSerialize::serialize(&self.procedural_action, builder);
        let procedural_action_exception_vec =
            FlatSerialize::serialize(&self.procedural_action_exception, builder);

        fb::HostnameSpecificRules::create(
            builder.raw_builder(),
            &fb::HostnameSpecificRulesArgs {
                unhide: Some(unhide),
                uninject_script: Some(uninject_script_vec),
                procedural_action: Some(procedural_action_vec),
                procedural_action_exception: Some(procedural_action_exception_vec),
            },
        )
    }
}
...
impl CosmeticFilterCache {
    ...
    pub fn hostname_cosmetic_resources(
        ...
        for hash in hashes.iter() {
            ...
            if let Some(rules_iterator) = hostname_rules_view.get(**hash) {
                for (_, hostname_rules) in rules_iterator {
        ...
        for hash in hashes.iter() {
            if let Some(rules_iterator) = hostname_rules_view.get(**hash) {
                for (_, hostname_rules) in rules_iterator {
                    ...
                    if let Some(procedural_exceptions) =
                        hostname_rules.procedural_action_exception()
                    {
...
impl<'a, B: Builder<'a>> FlatSerialize<'a, B> for CosmeticFilterCacheBuilder {
    type Output = WIPOffset<fb::CosmeticFilters<'a>>;
    fn serialize(&self, builder: &mut B) -> WIPOffset<fb::CosmeticFilters<'a>> {
        let (complex_class_rules_index, complex_class_rules_values) =
            self.complex_class_rules.finish(builder);
        let (complex_id_rules_index, complex_id_rules_values) =
            self.complex_id_rules.finish(builder);

        // Handle top-level hostname hide and inject_script for better deduplication
        let (hostname_hide_index, hostname_hide_values) = self.hostname_hide.finish(builder);
        let (hostname_inject_script_index, hostname_inject_script_values) =
            self.hostname_inject_script.finish(builder);

        // Handle remaining rule types through HostnameSpecificRules
        let (hostname_index, hostname_values) = self.specific_rules.finish(builder);

        let simple_class_rules = Some(FlatSerialize::serialize(&self.simple_class_rules, builder));
        let simple_id_rules = Some(FlatSerialize::serialize(&self.simple_id_rules, builder));
        let misc_generic_selectors = Some(FlatSerialize::serialize(
            &self.misc_generic_selectors,
            builder,
        ));
