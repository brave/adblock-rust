...
pub fn make_flatbuffer_from_rules(
    ...
    let flat_network_filters = FlatSerialize::serialize(network_builder, &mut builder);

    let cosmetic_builder = CosmeticFilterCacheBuilder::from_rules(cosmetic_rules);
    let flat_cosmetic_filters = FlatSerialize::serialize(cosmetic_builder, &mut builder);
