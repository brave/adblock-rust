...
impl<'a> Default for EngineFlatBuilder<'a> {
    fn default() -> Self {
        Self {
...
impl<'a> Builder<'a> for EngineFlatBuilder<'a> {
...
impl<'a> FlatSerialize<'a, EngineFlatBuilder<'a>> for NetworkFilter {
    ...
    fn serialize(&self, builder: &mut EngineFlatBuilder<'a>) -> WIPOffset<fb::NetworkFilter<'a>> {
...
impl NetworkRulesBuilder {
    ...
    pub fn serialize<'a>(
        ...
        let flat_network_rules: Vec<_> = self
            .lists
            .iter()
...
// impl<'a, B: Builder<'a>> FlatSerialize<'a, B> for Vec<NetworkFilterListBuilder> {
//     type Output = WIPOffset<Vector<'a, ForwardsUOffset<fb::NetworkFilterList<'a>>>>;
//     fn serialize(&self, builder: &mut B) -> <Self as FlatSerialize<'a,B>>::Output {
//         let mut flat_network_rules = vec![];

//         for mut list in self.into_iter() {
...
pub fn make_flatbuffer_from_rules(
    ...
    let network_builder = NetworkRulesBuilder::from_rules(network_filters, optimize);
    let flat_network_filters = network_builder.serialize(&mut builder);

    let cosmetic_builder = CosmeticFilterCacheBuilder::from_rules(cosmetic_rules);
