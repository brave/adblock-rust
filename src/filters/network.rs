                ...
                match option {
                    NetworkFilterOption::Domain(mut domains) => {
                        ...
                        if !opt_domains_array.is_empty() {
                            ...
                            let mut pairs: Vec<_> = opt_domains_array.into_iter().zip(opt_domains_strings_array.into_iter()).collect();
                            pairs.sort_by_key(|(hash, _)| *hash);
                            let (sorted_hashes, sorted_strings): (Vec<_>, Vec<_>) = pairs.into_iter().unzip();
                            opt_domains = Some(sorted_hashes);
                            opt_domains_strings = Some(sorted_strings);
                        }
                        if !opt_not_domains_array.is_empty() {
                            // Sort both arrays together to maintain correspondence between hashes and strings
                            let mut pairs: Vec<_> = opt_not_domains_array.into_iter().zip(opt_not_domains_strings_array.into_iter()).collect();
                            pairs.sort_by_key(|(hash, _)| *hash);
                            let (sorted_hashes, sorted_strings): (Vec<_>, Vec<_>) = pairs.into_iter().unzip();
                            opt_not_domains = Some(sorted_hashes);
                            opt_not_domains_strings = Some(sorted_strings);
                        }
                    }
