...
mod legacy_rule_parsing_tests {
    ...
    const EASY_PRIVACY: ListCounts = ListCounts {
        filters: 55501 - 828, // total - exceptions
        cosmetic_filters: 34,
        exceptions: 828,
        duplicates: 2,
    };
    ...
    fn check_list_counts(
        ...
        // Some filters are pointed at by multiple tokens, increasing the total number of items
        let map_size = |list: crate::network_filter_list::NetworkFilterList<'_>| {
            list.get_filter_map().total_size()
                ...
                + list.get_opt_to_domains_map().total_size()
        };
        assert!(
